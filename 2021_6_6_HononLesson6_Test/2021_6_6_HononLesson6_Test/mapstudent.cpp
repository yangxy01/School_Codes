...
//        {
//            cin >> no >> name;
//            if (mapStud.insert(pair<int, string>(no, name)).second == false)  //插入不成功，即学号已存在，键值不唯一
...
//        {
//            cin >> no;
//            auto reit = mapStud.equal_range(no);  //返回pair: [指向键等于key的元素的迭代器，指向key的下一个元素迭代器）
//            if (reit.first == reit.second)   //未找到，上界等于下界
//            {
